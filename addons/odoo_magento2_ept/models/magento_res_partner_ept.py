...
key_list = ['name', 'street', 'street2', 'city', 'zip', 'phone', 'state_id', 'country_id',
            'company_name']
key_list_without_company = ['name', 'street', 'street2', 'city', 'zip', 'phone', 'state_id', 'country_id', 'parent_id']
            ...
            if address.get('default_billing', False):
                ...
                if parent_partner.name == f"{address.get('firstname')} {address.get('lastname')}" and not (parent_partner.street or parent_partner.street2 or parent_partner.city or parent_partner.phone or parent_partner.zip):
                    ...
                    if company_name:
                        if instance.import_customer_as_company:
                            company_partner = self.env['res.partner'].search([('name', '=', company_name)], limit = 1)
                ...
                if not partner:
                    ...
                    if company:
                        ...
                        if instance.import_customer_as_company:
                            company_id = self.env['res.partner'].search([('name', '=', company)], limit = 1)
                            if company_id:
        ...
        for address in data.get('addresses'):
            if not address.get('default_billing', False):
                ...
                if instance.import_customer_as_company:
                    ...
                    partner = self.env['res.partner']._find_partner_ept(values, key_list=key_list)
                # if (partner.parent_name == values.get('company_name',
