...
class MagentoInstance(models.Model):
    ...
    magento_url = fields.Char(string='URLs', required=False, help="URL of Magento")
    magento_admin_url = fields.Char(string='Admin URL', required=False, help="URL of Magento Backend")
    warehouse_ids = fields.Many2many('stock.warehouse', 'magento_instance_warehouse_ids_rel',
                                     string="Warehouses",
                                     required=True,
    ...
    magento_website_ids = fields.One2many(comodel_name='magento.website',
                                          ...
                                          string='Magento Websites', readonly=True, help="Magento Websites")
    ...
    access_token = fields.Char(string="Access Token", help="Magento Access Token")
    ...
    activity_user_ids = fields.Many2many('res.users', 'magento_instance_activity_user_ids_rel',
    ...
    magento_weight_uom_id = fields.Many2one("uom.uom", string="Magento Weight Unit",
                                            domain=lambda self: [
                                                ("category_id", "=", self.env.ref("uom.product_uom_categ_kgm").id)],
                                            help="Select Weight unit same as Magento Store for setting "
    ...
    import_order_on = fields.Selection([("created_at", "Created At"), ("updated_at", "Updated At")],
                                       copy=False, default="updated_at")
    import_product_on = fields.Selection([("created_at", "Created At"), ("updated_at", "Updated At")],
                                         copy=False, default="updated_at")
    magento_import_attribute_page_count = fields.Integer(string="Magento Import Attribute Page Count",
                                                         default=1,
                                                         help="It will fetch attribute of Magento "
                                                              "from given page numbers.")
            ...
            if magento_storeview_id != 0:
                ...
                if not storeview:
                    name, language = self.get_store_view_language_and_name(stores,
                                                                           ...
                                                                           storeview_data)
                    storeview = storeview_obj.create({
                        'name': name,
                ...
                if storeview:
                    sales_team = self.find_sales_team(storeview)
                    storeview.write({
                        'base_media_url': storeview_data.get('base_media_url'),
                        'magento_storeview_code': storeview_data.get('code'),
    ...
    def _scheduler_import_shipped_sale_orders(self, args=None):
        ...
        if magento_instance_id:
            ...
            if last_order_import_date:
                last_order_import_date = last_order_import_date - timedelta(hours=10)
            ...
            magento_order_data_queue_obj.create_order_queues(
                ...
                to_date=to_date,
                status='complete',
                import_order_on=instance.import_order_on,
            )
    ...
    def _scheduler_import_unshipped_sale_orders(self, args=None):
        ...
        if magento_instance_id:
            ...
            if last_unshipped_order_import_date:
                last_unshipped_order_import_date = last_unshipped_order_import_date - timedelta(hours=10)
    ...
    def _scheduler_import_cancel_sale_orders(self, args=None):
        ...
        if magento_instance_id:
            ...
            if last_cancel_order_import_date:
                last_cancel_order_import_date = last_cancel_order_import_date - timedelta(hours=10)
    ...
    def _scheduler_import_product(self, args=None):
        ...
        is_update = args.get('do_not_update_Existing_product', False)
        if instance_id:
            instance = instance.browse(instance_id)
            from_date = ''
            if instance.last_product_import_date:
                from_date = instance.last_product_import_date - timedelta(hours=10)
            to_date = datetime.now()
    ...
    def get_refund(self, instance, website_id=0, state=('draft', 'posted', 'cancel')):
        ...
        if website_id:
            ...
            self._cr.execute(query, (website_id, instance.id, state))
        else:
            query = self._prepare_refund_query(instance, state)
            self._cr.execute(query, (instance.id, state))
    ...
    def get_total_products(self, record, exported, product_type=False):
        ...
        self._cr.execute(main_sql, (record.id,))
    ...
    def get_customers(self, record):
        ...
        main_sql = """select DISTINCT(rp.id) as partner_id from res_partner as rp
                    ...
                    where mp.magento_instance_id = %s"""
    ...
    def get_magento_invoice_records(self, state):
        ...
        self._cr.execute(invoice_query, (self.id, state))
        result = self._cr.dictfetchall()
    ...
    def open_logs(self, record_id):
        ...
        """
        view = self.env.ref('odoo_magento2_ept.action_magento_process_log_lines_ept').sudo().read()[0]
    ...
    def get_compared_week_data(self, record):
        ...
        self._cr.execute("""select sum(amount_untaxed) as current_week from sale_order
                                ...
                                magento_instance_id=%s and state in ('sale','done')""", (record.id,))
    ...
    def get_compared_month_data(self, record):
        ...
        self._cr.execute("""select sum(amount_untaxed) as current_month from sale_order
                                where date(date_order) >= (select date_trunc('month', date(current_date)))
                                and magento_instance_id=%s and state in ('sale','done')""", (record.id,))
    ...
    def create_schedule_activity(self, queue, note):
        if queue.process_count == 3 and queue.instance_id.is_create_activity:
            ...
            # queue.write({'is_action_require': True}) # removed because put it in separate method
            # Commit the record to reduce concurrent issue.
            self._cr.commit()
        return True

    def enable_action_required_message(self, queue):
        if queue:
    ...
    def find_sales_team(self, storeview):
        """
        sales team find if not found then create new sales team instance and store wise.
        :param storeview: storeview browse object
    ...
    def open_reset_credentials_wizard(self):
        ...
        @author: Nikul Alagiya on Date 14-Sep-2022.
        """
        view_id = self.env.ref('odoo_magento2_ept.view_reset_credentials_form').id
        action = self.env.ref('odoo_magento2_ept.res_config_action_magento_instance_ept').read()[0]
        action.update({"name": "Reset Credentials",
                       "context": {'magento_instance_id': self.id,
                                   "default_magento_url": self.magento_url,
                                   "default_magento_admin_url": self.magento_admin_url,
