...
class SaleOrder(models.Model):
    ...
    magento_order_id = fields.Char(string="order Id", copy=False, help="Magento Order Id")
    ...
    def _cancel_order_exportable(self):
        ...
        if (self.invoice_ids and True in self.invoice_ids.mapped('is_exported_to_magento')) or \
                (self.picking_ids and self.picking_ids.filtered(
                    lambda x: x.state == 'done' and x.is_exported_to_magento).ids) or self.is_order_cancel_in_magento:
    ...
    is_cancel_order_exportable = fields.Boolean(string="Is Invoice exportable",
                                                ...
                                                store=False)
    is_order_cancel_in_magento = fields.Boolean(string="Is order cancel in magento",
                                                store=True)
    ...
    def __find_order_warehouse(item, log_line, line_id):
        ...
        if not item.get('website').warehouse_id.id:
            ...
            """)
            log_line.create_common_log_line_ept(message=message, order_ref=item['increment_id'],
                                                magento_order_data_queue_line_id=line_id,
                                                magento_instance_id=item.get('instance_id').id)
            is_processed = False
        return is_processed
    ...
    def __find_order_tax(self, item, instance, log_line, line_id):
        order_line = self.env['sale.order.line']
    ...
    def cancel_order_from_magento(self):
        ...
        result = False
        try:
            result = super(SaleOrder, self).with_context(disable_cancel_warning=True).action_cancel()
        ...
        if not result:
            ...
            log_line.create_common_log_line_ept(message=message, module='magento_ept', res_id=self.id,
                                                operation_type='import', model_name='sale.order',
                                                magento_instance_id=self.magento_instance_id.id, order_ref=self.name)
        self._cr.commit()
        return True
    ...
    def cancel_order_in_magento(self):
        ...
        if magento_order_id:
            ...
            try:
                ...
                if result:
                    self.is_order_cancel_in_magento = True
            except Exception:
    ...
    def validate_and_paid_invoices_ept(self, work_flow_process_record):
        """
        Inherit this method from common connector:sale order
        According to the workflow configuration, It will create invoices, validate them and register payment.
        but changes is flow is, if magento order status is pending then only create draft invoice, do not validate
        and register payment
        changes by @Ketan Chauhan On 04.Dec.2023 when magento migration in v17
        ...
        if work_flow_process_record.create_invoice:
            ...
            if work_flow_process_record.register_payment:
                is_invoice = True
                if self._context.get('is_invoice') is not None:
                    is_invoice = self._context.get('is_invoice')
                if self.magento_instance_id and not is_invoice:
                    return True
