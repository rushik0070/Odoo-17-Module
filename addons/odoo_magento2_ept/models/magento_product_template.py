...
class MagentoProductTemplate(models.Model):
    ...
    magento_website_ids = fields.Many2many('magento.website', 'magento_product_template_magento_website_rel', string='Magento Websites',
    ...
    product_type = fields.Selection(
        [('simple', 'Simple Product'), ('configurable', 'Configurable Product'),
         ...
         ], string='Product Type', help='Magento Product Type', default='simple')
    magento_sku = fields.Char(string="Product SKU", help="Magento Product SKU")
    description = fields.Html(string="Product Description", help="Description", translate=True)
    short_description = fields.Html(string='Product Short Description', help='Short Description',
                                    translate=True)
    ...
    category_ids = fields.Many2many("magento.product.category", 'magento_product_template_category_ids_rel',
                                    string="Categories", help="Magento Categories")
    ...
    def __search_layer_template(self, line, item, values):
        ...
        if not template:
            if item.get('extension_attributes', False) and item.get('extension_attributes').get('child_products', False):
        ...
        if template:
            if 'is_order' not in list(self.env.context.keys()):
                if not line.do_not_update_existing_product and not self.env.context.get('is_child'):
    ...
    def _create_configurable_variant(self, line, item, m_template, data):
        ...
        is_verify = self._verify_attribute_count(line, item, o_template, e_attributes, m_template)
        if is_verify:
            ...
            for product in e_attributes.get('child_products', list()):
                for attribute in product.get('simple_product_attribute', list()):
                    attribute_dict = m_attribute.search_attribute_by_value(instance, attribute,
                                                                           item.get(
                                                                               'attribute_set_id'), e_attributes)
                    self.__add_variant(o_template, attribute_dict, product, instance)
            ...
            is_verify = self._update_variant_sku(line, m_template, item, child_products, data)
        return is_verify

    def _verify_attribute_count(self, line, item, o_template, e_attributes, m_template):
        ...
        if o_attr_count > 0 and o_attr_count != m_attr_count:
            ...
            if field_name == 'magento_import_product_queue_line_id':
                ...
                log_line.create_common_log_line_ept(message=message,
                                                    magento_order_data_queue_line_id=line.id, model_name=self._name,
                                                    magento_instance_id=line.instance_id.id)
            return False
        for product in e_attributes.get('child_products', list()):
            for attribute in product.get('simple_product_attribute', list()):
                attribute_dict = m_attribute.search_attribute_by_value(instance, attribute,
                                                                       item.get(
                                                                           'attribute_set_id'), e_attributes)
                attr_line = self.env['product.template.attribute.line']
                attr_line = attr_line.search([('product_tmpl_id', '=', o_template.id),
                                    ('attribute_id', '=', attribute_dict.get('attribute_id'))])
                value_id = attribute_dict.get('value_id')
                if value_id:
                    if attr_line and value_id not in attr_line.value_ids.ids:
                        odoo_product = odoo_product.search([("default_code", "=", product.get('simple_product_sku'))], limit=1)
                        if odoo_product:
                            continue
    ...
    def _update_variant_sku(self, line, m_template, item, child_products, data):
        ...
        m_variants = self.env['magento.product.product'].search([('magento_tmpl_id', '=', m_template.id)])
        for product in child_products:
            variant = self.env['product.product'].search(
                [("default_code", "=", product.get('simple_product_sku'))], limit=1)
            # if variant in variants and self.__find_child(variant.product_template_variant_value_ids, child_products):
            m_variant = self.env['magento.product.product'].search(
                [("magento_sku", "=", product.get('simple_product_sku'))], limit=1)
            if not m_variant:
                child, m_variant = self.__find_child(m_variants, product)
                if m_variant:
                    m_variant.write({
                        'magento_sku': child.get('simple_product_sku', '')
                    })
            if not variant:
                child, variant = self.__find_child(variants, product)
                if not variant:
                    return False
                variant.write({
                ...
                self._map_variant_in_layer(variant, m_template, product, data, item)
                prices = product.get('website_wise_product_price_data', list())
                if 'is_order' not in list(self.env.context.keys()):
                    if not line.do_not_update_existing_product and variant:
                        self.update_price_list(line, variant, prices)
        return True
    ...
    @staticmethod
    def __find_child(variants, child):
    ...
    def __add_variant(self, template, data, product, instance):
        ...
        if value_id:
            if not line and instance.auto_create_product:
                line.create({
                    'product_tmpl_id': template.id,
                    'attribute_id': data.get('attribute_id'),
                    'value_ids': [(4, value_id)]
                })
            elif line and value_id not in line.value_ids.ids:
                odoo_product = self.env['product.product'].search([("default_code", "=", product.get('simple_product_sku'))], limit=1)
                if odoo_product:
                    return True
                elif instance.auto_create_product:
                    line.write({
                        'value_ids': [(4, value_id)]
                    })
        return True
            ...
            for image in item.get('media_gallery_entries', list()):
                ...
                try:
                    ...
                    log_line.create_common_log_line_ept(message=message, order_ref=item.get('increment_id', ''),
                                                        res_id=line.queue_id.id, model_name=self._name,
    ...
    def __prepare_template_values(self, instance, item, data, product=False, template=False):
        values = {
            'magento_product_name': item.get('name') if item.get('name') else item.get('sku'),
            'magento_instance_id': instance.id,
    ...
    def get_magento_attribute_set(product, product_type, is_it_child, attribute_set_id,
        ...
        if not attribute_set:
            ...
            log_line.create_common_log_line_ept(message=message, default_code=product.magento_sku,
                                                operation_type='export', model_name='magento.product.template')
    ...
    def get_scope_wise_product_price(instance, log_line, product, price_list, cost_price=False):
        ...
        :param instance:  Magento Instance Object
        :param log_line: Common Log line object
    ...
    def prepare_configurable_option_vals(self, conf_product_sku, log_line, instance,
                                         product,
                                         attribute_set_id):
        """
        ...
        for line in attribute_line:
            ...
            if not magento_att:
                # if magento attribute is not found then skip product with log
                # previously available this functionality inv16, but stop in v17
                message = "Can't find attribute in "
                "Magento Attribute list : %s" % line.attribute_id.name
                log_line.create_common_log_line_ept(message=message,
                                                    operation_type='export', model_name='magento.product.template',
                                                    magento_instance_id=instance.id)
                return True
            ...
            if magento_att and not magento_att.magento_attribute_id:
                ...
                #     magento_att, instance, log_line, conf_product_sku, attribute_options)
                # if skip:
                #     return True
            ...
            if not is_attribute_assigned and attribute_set_id:
                # if magento attribute and attribute is not assigned then skip with log
                # previously available this functionality inv16, but stop in v17
                message = "Can't find attribute and attribute set as assigned in "
                "Magento Attribute Set  : %s" % line.attribute_id.name
                log_line.create_common_log_line_ept(message=message,
                                                    operation_type='export', model_name='magento.product.template',
                                                    magento_instance_id=instance.id)
                return True
                # skip = self.assign_attribute_in_attribute_set(
                #     instance, product, attribute_set_id, log_line,
    ...
    def export_attribute_in_magento(self, magento_att, line, conf_product_sku, instance, log_line):
        '''
        ...
        try:
            ...
            log_line.create_common_log_line_ept(message=message, default_code=conf_product_sku,
                                                operation_type='export', model_name='magento.product.template',
                                                magento_instance_id=instance.id)
            return True
    ...
    def export_product_attribute_in_magento(
            self, magento_attribute, instance, log_line, sku, attribute_options):
        ...
        try:
            ...
            log_line.create_common_log_line_ept(message=message, default_code=sku,
                                                operation_type='export', model_name='magento.product.template',
                                                magento_instance_id=instance.id)
    ...
    def export_attribute_options(self, magento_attribute, instance,
        ...
        :param instance:  magento instance
        :param log_line: log_line object
        ...
        for attribute_option in attribute_options:
            ...
            try:
                ...
                # Changes done on 30.11.2023 by Ketan Chauhan
                # if new option creating in magento store, with create magento layer option
                if response:
                    option = self.env['magento.attribute.option']
                    attribute_vals = {'options': [{'label': attribute_option.get('label'), 'value': response.strip()}]}
                    option.create_attribute_option(attribute_vals, magento_attribute,
                                                   magento_attribute.odoo_attribute_id)
                # done changes
    ...
    def bind_simple_with_configurable_product(
            instance, product_magento_sku_list, conf_product_sku, log_line):
        """
        ...
        :param log_line: log book object
        :return:
        """
        for product_magento_sku in product_magento_sku_list:
            try:
                ...
                """
                log_line.create_common_log_line_ept(message=message, default_code=product_magento_sku,
                                                    operation_type='export', model_name='magento.product.template',
                                                    magento_instance_id=instance.id)
        return True
    ...
    @staticmethod
    def update_product_base_price(instance, url, log_line, price_payload):
        """
    ...
    def prepare_export_stock_data(self, product_stock, instance, log_line, layer_ids, source_code=False,
                                  msi=False):
        """
        ...
        for product_id, stock in product_stock.items():
            ...
            if product:
                if product.get('type') != 'product':
                    consumable.append(product.get('magento_sku'))
                else:
                    if msi:
                        stock_data.append({
            ...
            if consumable:
                m_product.create_export_product_process_log(consumable, log_line, instance)
        return stock_data
