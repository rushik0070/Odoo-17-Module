...
class MagentoWebsite(models.Model):
    ...
    magento_website_id = fields.Char(string="Magento Website", help="Website Id")
    ...
    warehouse_id = fields.Many2one(comodel_name='stock.warehouse', string='Magento Warehouse',
    ...
    def get_total_products(self, record, exported, product_type=False):
        ...
        self._cr.execute(main_sql, (record.id, record.magento_instance_id.id))
    ...
    def get_magento_invoice_records(self, state):
        ...
        self._cr.execute(magento_invoice_query, (self.id, self.magento_instance_id.id, state))
        result = self._cr.dictfetchall()
    ...
    def get_magento_picking_records(self, state):
        ...
        and SP.state in ('%s')
        """
        self._cr.execute(magento_picking_query, (self.magento_instance_id.id, self.id, state))
        result = self._cr.dictfetchall()
        view = \
