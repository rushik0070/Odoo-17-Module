...
class ResConfigSettings(models.TransientModel):
    ...
    magento_team_id = fields.Many2one('crm.team', string='Magento Sales Team', help="Magento Sales Team")
    ...
    magento_website_warehouse_id = fields.Many2one(
        STOCK_WAREHOUSE,
        string='Magento Website Warehouse',
        help='Warehouse to be used to deliver an order from this website.'
    )
    warehouse_ids = fields.Many2many(
        STOCK_WAREHOUSE,
        'magento_config_settings_warehouse_ids_rel',
        string="Warehouses",
        help='Warehouses used to compute the stock quantities, when only single warehouse is in Magento and perform '
             'export product stock operation.'
    )
    ...
    allow_import_image_of_products = fields.Boolean(
        ...
        default=False,
        help="If checked, import product images along with product from Magento while import products."
    )
    # Import Product Stock
    is_import_product_stock = fields.Boolean(
        'Is Import Magento Product Stock?',
        default=False,
        help="If checked, import Product Stock from Magento to Odoo."
    )
    import_stock_warehouse = fields.Many2one(
        STOCK_WAREHOUSE,
        string="Import Product Stock Warehouse",
        help="Warehouse used to store stock quantities, when only single warehouse is in Magento and perform import"
             " product stock operation."
    )
    magento_stock_field = fields.Selection([
        ...
        help="Select inventory field for Magento products "
    ...
    auto_create_product = fields.Boolean(
        ...
        default=False,
        help="- This option creates a new product in your Odoo catalog if system couldn't find the product based on SKU/Internal Reference/Default Product Code."
    ...
    invoice_done_notify_customer = fields.Boolean(
        ...
        default=False,
        help="If checked,it will send email to the customer while Invoice is created in Magento."
    )
    ...
    is_multi_warehouse_in_magento = fields.Boolean(
        ...
        default=False,
        help="If checked, Multi Inventory Sources are used in Magento to keep product stock"
    )
    ...
    magento_invoice_tax_account_id = fields.Many2one("account.account", string="Invoice Tax Account For Magento Tax",
                                                     help='Set the invoice tax account base on this account we will set'
    ...
    def onchange_magento_instance_id(self):
        ...
        if magento_instance_id:
            self.write({
                ...
                'magento_weight_uom_id': magento_instance_id.magento_weight_uom_id or False,
                'import_order_on': magento_instance_id.import_order_on,
                'import_product_on': magento_instance_id.import_product_on,
    ...
    def onchange_magento_website_pricelist_id(self):
        if self.magento_website_id:
            self.magento_website_id.write(
                {'pricelist_id': self.magento_website_pricelist_id.id})
    ...
    def onchange_magento_website_id(self):
        ...
        if magento_website_id:
            if magento_website_id.pricelist_id.id:
    ...
    def execute(self):
        ...
        if not self.magento_show_net_profit_report and exist_module:
    ...
    def write_instance_vals(self, magento_instance_id):
        ...
        values.update({
            ...
            'magento_weight_uom_id':  self.magento_weight_uom_id or False,
            'import_order_on': self.import_order_on,
            'import_product_on': self.import_product_on,
    ...
    def onchange_is_magento_digest(self):
        ...
        if self.is_magento_digest:
            ...
            vals = self.prepare_val_for_magento_digest()
            if digest_exist:
    ...
    def prepare_val_for_magento_digest(self):
