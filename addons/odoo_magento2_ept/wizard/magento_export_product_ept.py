...
class MagentoExportProductEpt(models.TransientModel):
    ...
    magento_instance_ids = fields.Many2many('magento.instance', 'magento_export_product_instance_ids_rel', string="Instances",
    ...
    def process_export_products_in_magento(self):
        ...
        for instance in instances:
            ...
            for m_template in m_templates:
                ...
                if attribute_set_id:
                    if m_template.product_type == 'simple':
                        self.export_simple_product(instance, m_template, attribute_set_id, log_line)
                    else:
    ...
    def export_simple_product(self, instance, m_template, attribute_set_id, log_line):
        """
        ...
        if product_tmpl:
            for product in product_tmpl:
                ...
                self.create_product_in_magento(
                    instance, product, attribute_set_id, log_line, product_type='simple')
        return True
    ...
    def export_configurable_product(self, instance, m_template, attribute_set_id,
        ...
        if product_tmpl:
            for product in product_tmpl:
                ...
                self.create_product_in_magento(instance, product, attribute_set_id, log_line,
                                               product_type='configurable')
                if product.attribute_line_ids:
                    conf_product_sku = product.magento_sku if product.magento_sku else product.magento_product_name
                    configurable_opt_vals = magento_product_obj.prepare_configurable_option_vals(
                        conf_product_sku, log_line, instance, product, attribute_set_id)
    ...
    def __find_attribute_for_product(self, product, instance, attribute_set_id, log_line):
        """
    ...
    def process_update_products_in_magento(self):
        ...
        for instance in instances:
            log_line = self.env['common.log.lines.ept']
            self.__find_not_synced_m_templates(instance, update_ids, log_line)
            m_templates = self.__find_magento_templates(update_ids, instance)
            if m_templates:
                start = time.time()
                self.__update_simple_product(instance, m_templates, log_line)
                self.__update_configurable_product(instance, m_templates, log_line)
                end = time.time()
    ...
    def __find_not_synced_m_templates(self, instance, update_ids, log_line):
        ...
        for magento_tmpl in m_templates:
            ...
            log_line.create_common_log_line_ept(message=message, default_code=magento_tmpl.magento_sku,
                                                operation_type='export', model_name='magento.product.template',
                                                magento_instance_id=instance.id)
        return True

    def __update_simple_product(self, instance, m_templates, log_line):
        product_tmpl = m_templates.filtered(lambda x: x.product_type == 'simple')
        for simple_product in product_tmpl:
            self.__update_product(instance, simple_product, log_line)
        return True

    def __update_configurable_product(self, instance, m_templates, log_line):
        ...
        if len(product_tmpl) > 0:
            for conf_product in product_tmpl:
                ...
                for website_id in website_ids:
                    product_dict = self.__prepare_conf_product_dict(conf_product, log_line, website_id)
                ...
                for m_variant in conf_product.magento_product_ids:
                    self.__update_product(instance, m_variant, log_line, True)
        return True
    ...
    def __update_product(self, instance, m_template, log_line, is_child=False):
        m_template_obj = self.env['magento.product.template']
        ...
        if website_ids:
            for website_id in website_ids:
                product_dict = self.__prepare_update_product_dict(m_template, log_line, is_child, website_id)
                # update product weight in magento
                if m_template._name == 'magento.product.template':
                    weight = self.convert_weight_by_uom(m_template.odoo_product_template_id.weight, instance)
                else:
                    weight = self.convert_weight_by_uom(m_template.odoo_product_id.weight, instance)
                if self.m_update_basic_details:
                    product_dict.get('product').update({'weight': str(weight)})
    ...
    def convert_weight_by_uom(self, weight, instance, import_process=False):
        ...
        @author: Yagnik Joshi on Date 31-May-2023.
        ...
        """
        magento_weight_uom = instance.magento_weight_uom_id
        product_weight_uom = self.env["product.template"]._get_weight_uom_id_from_ir_config_parameter()

        if magento_weight_uom != product_weight_uom:
            if import_process:
                weight = magento_weight_uom._compute_quantity(weight, product_weight_uom)
            else:
                weight = product_weight_uom._compute_quantity(weight, magento_weight_uom)
        return weight
    ...
    def export_stock_in_magento_ept(self):
        ...
        for instance in instances:
            ...
            self.export_product_stock_magento(instance, product_ids, log_line)
    ...
    def export_product_stock_magento(self, instance, product_ids, log_line):
        ...
        if instance.magento_version in ['2.1', '2.2'] or not instance.is_multi_warehouse_in_magento:
            ...
            # will be called.
            self.export_stock_non_msi(instance, product_ids, log_line, layer_ids)
        else:
            self.export_stock_with_msi(instance, product_ids, log_line, layer_ids)
        return True
    ...
    def _filter_export_stock_products(self, instance, product_ids):
        ...
        datas = []
        o_product = o_product.search_read([('id', 'in', product_ids), ('active', '=', True)], fields=['type'])
        if o_product:
            for product in m_products:
                product_id = product.get('odoo_product_id')[0]
                prod_type = [p.get('type') for p in o_product if p.get('id') == product_id]
                if product.get('magento_sku') and prod_type:
                    datas.append({
    ...
    def export_stock_non_msi(self, instance, product_ids, log_line, layer_ids):
        """
        ...
        if product_stock:
            stock_data = m_template.prepare_export_stock_data(product_stock=product_stock,
                                                              instance=instance, log_line=log_line,
                                                              layer_ids=layer_ids)
            if stock_data:
    ...
    def export_stock_with_msi(self, instance, product_ids, log_line, layer_ids):
        """
        ...
        for location in locations:
            ...
            if product_stock:
                stock_data += m_template.prepare_export_stock_data(product_stock=product_stock,
                                                                   instance=instance,
                                                                   log_line=log_line, layer_ids=layer_ids,
    ...
    def create_product_in_magento(self, instance, product, attribute_set_id, log_line,
        ...
        attribute_set = m_template.get_magento_attribute_set(
            product, product_type, is_it_child, attribute_set_id, log_line)
        data = self.prepare_product_data(product, is_it_child, product_type, attribute_set,
                                         website_ids)
        if m_product._name == 'magento.product.template':
            weight = self.convert_weight_by_uom(m_product.odoo_product_template_id.weight, instance)
        else:
            weight = self.convert_weight_by_uom(m_product.odoo_product_template_id.product_variant_id.weight, instance)
        data.get('product').update({'weight': str(weight)})
    ...
    def __create_product_website_vise(self, instance, conf_simple_product, website_ids, product,
        ...
        for website in website_ids:
            ...
            for store in m_store_view:
                price = self.__get_product_price(instance, website, conf_simple_product, log_line)
                ...
                try:
                    ...
                    message = "%s \nNot able to create new Simple  Product in Magento. SKU : %s" % (error, product.magento_sku)
                    log_line.create_common_log_line_ept(message=message, default_code=product.magento_sku,
                                                        operation_type='export', model_name='magento.product.template',
                                                        magento_instance_id=instance.id)
                    return False
    ...
    @staticmethod
    def __set_websites_images_in_magento(instance, product, data, log_line):
        sku = product.magento_sku if product.magento_sku else product.magento_product_name
        try:
    ...
    def __get_product_price(self, instance, website, m_template, log_line):
        m_template_obj = self.env['magento.product.template']
        product_price = 0
        currency = website.magento_base_currency.id
        if self.is_set_price:
            if instance.catalog_price_scope == 'global':
                product_price = m_template_obj.get_scope_wise_product_price(
                    instance, log_line, m_template, instance.pricelist_id)
            else:
                price_list = website.pricelist_id.filtered(lambda x: x.currency_id.id == currency)
                if price_list:
                    product_price = m_template_obj.get_scope_wise_product_price(instance, log_line,
                                                                                m_template,
                                                                                price_list)
    ...
    def fine_product_attributes(self, o_product, instance, attribute_set_id,
        ...
        for value_id in o_product.product_template_attribute_value_ids:
            ...
            if o_product.odoo_product_id.name == o_product.magento_product_name:
        ...
        self.create_product_in_magento(instance, o_product, attribute_set_id, log_line,
                                       product_type='simple',
                                       is_it_child=True,
