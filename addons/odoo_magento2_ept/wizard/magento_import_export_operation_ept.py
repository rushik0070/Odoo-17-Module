...
class MagentoImportExportEpt(models.TransientModel):
    ...
    def on_change_operation(self):
        ...
        """
        if self.operations in ["import_configurable_products", "import_simple_products", "import_unship_sale_order",
                               "import_ship_sale_order", "import_cancel_orders", "import_customer"]:
    ...
    def import_cancel_order_operation(self, instance):
        ...
        :return:
        """
        kwargs = {'from_date': self.start_date, 'to_date': self.end_date, 'import_order_on': instance.import_order_on}
    ...
    def import_product_stock_operation(self, instance):
        ...
        if instance:
            ...
            if instance.magento_version in ['2.1',
                ...
                # will be called.
                is_log_line_exist = m_product.import_product_inventory(instance,
                                                                       self.auto_validate_stock)
            else:
                locations = m_locations.search([('magento_instance_id', '=', instance.id)])
                is_log_line_exist = m_product.import_product_multi_inventory(instance,
                                                                             locations,
                                                                             self.auto_validate_stock)
        if is_log_line_exist:
            view_ref = self.env.ref('common_connector_library.common_log_line_view_form').id
            return {
    ...
    def export_product_file_create(self, variants):
        ...
        return {
            ...
            'url': '/web/binary/magento_download_document?model=magento.import.export.ept&'
    ...
    def validate_product_dict(self, row, instance, row_number):
        ...
        if row.get('instance_id') != instance.id:
            ...
            log_line = log_line.create_common_log_line_ept(message=message, magento_instance_id=instance.id,
                                                           operation_type='export', model_name=self._name)
            return log_line
